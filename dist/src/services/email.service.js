...
class EmailService {
    resend;
    constructor() {
        this.resend = new resend_1.Resend(config_1.config.RESEND_API_KEY);
    }
    /**
     * Helper to send email with exponential backoff retries
     */
    async sendEmailWithRetry(to, subject, html, retries = 3, delay = 1000) {
        let attempt = 0;
        while (attempt < retries) {
            ...
            try {
                ...
                if (response.error) {
                    logger_1.logger.warn(`[Resend] Delivery attempt ${attempt}/${retries} failed: ${response.error.message}`, {
                    ...
                    logger_1.logger.info(`[Resend] Retrying email delivery (Attempt ${attempt + 1}/${retries})...`);
                }
                else {
                    // Success
                    logger_1.logger.info(`📧 [Resend] Email successfully dispatched to [${to}]`, {
                ...
                logger_1.logger.error(`[Resend] Delivery attempt ${attempt}/${retries} encountered error: ${err.message}`);
                if (attempt >= retries) {
                    throw err;
                }
                await new Promise((resolve) => setTimeout(resolve, delay * Math.pow(2, attempt - 1)));
    ...
    async sendWelcome(to, name) {
        ...
        try {
            ...
            logger_1.logger.error(`[EmailService] Failed to send Welcome email to [${to}]:`, error);
